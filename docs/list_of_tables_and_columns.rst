.. csv-table:: Tables, columns and comments
   ...
   **AutoProcScalingStatistics**,table,""
   ...
   ``ccAnomalous``,float,""
   ``resIOverSigI2``,float,"Resolution where I/Sigma(I) equals 2"
   ...
   **BLSample_has_Positioner**,table,""
   ``blSampleHasPositioner``,int,""
   ``blSampleId``,int,""
   ``positionerId``,int,""
   **BLSession**,table,""
   ``sessionId``,int,""
   ...
   **BLSubSample_has_Positioner**,table,""
   ``blSubSampleHasPositioner``,int,""
   ``blSubSampleId``,int,""
   ``positionerId``,int,""
   **Buffer**,table,""
   ``bufferId``,int,""
   ...
   **ContainerQueueSample**,table,""
   ...
   ``containerQueueId``,int,""
   ``blSubSampleId``,int,""
   ``status``,varchar,"The status of the queued item, i.e. skipped, reinspect. Completed / failed should be inferred from related DataCollection"
   ``startTime``,datetime,"Start time of processing the queue item"
   ``endTime``,datetime,"End time of processing the queue item"
   ``dataCollectionPlanId``,int,""
   ``blSampleId``,int,""
   ...
   **DiffractionPlan**,table,""
   ...
   ``qMin``,float,"minimum in qRange, unit: nm^-1, needed for SAXS"
   ``qMax``,float,"maximum in qRange, unit: nm^-1, needed for SAXS"
   ``reductionParametersAveraging``,enum,"Post processing params for SAXS"
   **EMMicroscope**,table,""
   ``emMicroscopeId``,int,""
   ...
   **ParticleClassification**,table,"Results of 2D or 2D classification"
   ...
   ``classDistribution``,float,"Provides a figure of merit for the class, higher number is better"
   ...
   **PDB**,table,""
   ...
   ``contents``,mediumtext,""
   ``code``,varchar,""
   ``source``,varchar,"Could be e.g. AlphaFold or RoseTTAFold"
   **PDBEntry**,table,""
   ``pdbEntryId``,int,""
   ``autoProcProgramId``,int,""
   ``code``,varchar,""
   ...
   **Position**,table,""
   ...
   ``Y``,double,""
   ``Z``,double,""
   **Positioner**,table,"An arbitrary positioner and its value, could be e.g. a motor. Allows for instance to store some positions with a sample or subsample"
   ``positionerId``,int,""
   ``positioner``,varchar,""
   ``value``,float,""
   ...
   **ProcessingPipeline**,table,"A lookup table for different processing pipelines and their categories"
   ``processingPipelineId``,int,""
   ``processingPipelineCategoryId``,int,""
   ``name``,varchar,""
   ``discipline``,varchar,""
   ``pipelineStatus``,enum,"Is the pipeline in operation or available"
   ``reprocessing``,tinyint,"Pipeline is available for re-processing"
   **ProcessingPipelineCategory**,table,"A lookup table for the category of processing pipeline"
   ``processingPipelineCategoryId``,int,""
   ``name``,varchar,""
   **Project**,table,""
   ``projectId``,int,""
   ...
   **PurificationColumn**,table,"Size exclusion chromotography (SEC) lookup table for BioSAXS"
   ...
   ``name``,varchar,""
   ``active``,tinyint,"1=active, 0=inactive"
   **RelativeIceThickness**,table,""
   ``relativeIceThicknessId``,int,""
   ``motionCorrectionId``,int,""
   ``autoProcProgramId``,int,""
   ``minimum``,float,"Minimum relative ice thickness, Unitless"
   ``q1``,float,"Quartile 1, unitless"
   ``median``,float,"Median relative ice thickness, Unitless"
   ``q3``,float,"Quartile 3, unitless"
   ``maximum``,float,"Minimum relative ice thickness, Unitless"
   ...
   **WorkflowType**,table,""
   ...
   ``comments``,varchar,""
   ``recordTimeStamp``,timestamp,""
   **XFEFluorescenceComposite**,table,"A composite XRF map composed of three XRFFluorescenceMapping entries creating r, g, b layers"
   ``xfeFluorescenceCompositeId``,int,""
   ``r``,int,"Red layer"
   ``g``,int,"Green layer"
   ``b``,int,"Blue layer"
   ``rOpacity``,float,"Red layer opacity"
   ``bOpacity``,float,"Red layer opacity"
   ``gOpacity``,float,"Red layer opacity"
   ``opacity``,float,"Total map opacity"
   ...
   **XRFFluorescenceMapping**,table,"An XRF map generated from an XRF Mapping ROI based on data from a gridscan of a sample"
   ...
   ``gridInfoId``,int,""
   ``dataFormat``,varchar,"Description of format and any compression, i.e. json+gzip for gzipped json"
   ``data``,longblob,"The actual data"
   ``points``,int,"The number of points available, for realtime feedback"
   ...
   **XRFFluorescenceMappingROI**,table,""
   ...
   ``blSampleId``,int,"ROIs can be created within the context of a sample"
   ``scalar``,varchar,"For ROIs that are not an element, i.e. could be a scan counter instead"
   **zc_ZocaloBuffer**,table,""
   ``AutoProcProgramID``,int,"Reference to an existing AutoProcProgram"
   ``UUID``,int,"AutoProcProgram-specific unique identifier"
   ``Reference``,int,"Context-dependent reference to primary key IDs in other ISPyB tables"
