.. csv-table:: Tables, columns and comments
   ...
   **BLSampleGroupType**,table,""
   ``blSampleGroupTypeId``,int,""
   ``name``,varchar,""
   ...
   **CalendarHash**,table,"Lets people get to their calendars without logging in using a private (hash) url"
   ...
   ``hash``,varchar,""
   ``beamline``,tinyint,""
   **Column**,table,"Experimental conditions lookup table for BioSAXS"
   ``columnId``,int,""
   ``name``,varchar,""
   ``active``,tinyint,"1=active, 0=inactive"
   ...
   **DiffractionPlan**,table,""
   ...
   ``robotPlateTemperature``,float,"units: "
   ``exposureTemperature``,float,"units: "
   ``qMin``,float,"minimum in qRange"
   ``qMax``,float,"maximum in qRange"
   ``reductionParametersAveraging``,enum,""
   **EMMicroscope**,table,""
   ``emMicroscopeId``,int,""
