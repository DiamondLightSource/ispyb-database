...
BEGIN
    IF p_id IS NOT NULL THEN
    	...
    	-- Authorise only if the person (p_authLogin) is a member of a session on the proposal.
				...
				SELECT c.dewarId, c.code, c.containerType, c.capacity, c.sampleChangerLocation, c.containerStatus, c.bltimeStamp, c.beamlineLocation, c.screenId, c.scheduleId, c.barcode, c.imagerId, c.sessionId, c.ownerId, c.requestedImagerId, c.requestedReturn, c.comments, c.experimentType, c.storageTemperature, c.containerRegistryId,
        cr.barcode "containerRegistryBarcode", cr.comments "containerRegistryComments", cr.recordTimestamp "containerRegistryRecordTimestamp",
        o.familyName "ownerFamilyName", o.givenName "ownerGivenName", o.emailAddress "ownerEmailAddress",
        pp.name "processingPipelineName", pp.discipline "processingPipelineDiscipline", pp.pipelineStatus "processingPipelinePipelineStatus", pp.reprocessing "processingPipelineReprocessing",
        ppc.name "processingPipelineCategoryName"
				FROM BLSample bls
          INNER JOIN Container c USING (containerId)
          INNER JOIN Dewar d USING (dewarId)
          ...
          LEFT OUTER JOIN ProcessingPipeline pp ON pp.processingPipelineId = c.priorityPipelineId
          LEFT OUTER JOIN ProcessingPipelineCategory ppc USING(processingPipelineCategoryId)
